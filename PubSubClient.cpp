...
boolean PubSubClient::connect(char *id, char *user, char *pass, char* willTopic, uint8_t willQos, uint8_t willRetain,
		char* willMessage) {
	if (!connected()) {
		int result = 0;
...
uint16_t PubSubClient::readPacket(uint8_t* lengthLength) {
	...
	*lengthLength = len - 1;
	...
	if (len < MQTT_MAX_PACKET_SIZE) {
		memset(buffer + len, 0, MQTT_MAX_PACKET_SIZE - len);
	}
